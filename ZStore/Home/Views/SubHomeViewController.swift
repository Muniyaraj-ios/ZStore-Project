...
//        let textEmpty = searchSectionStack.isHidden//searchTxt.text?.isEmpty ?? true
    ...
    private func compProductLayout()-> NSCollectionLayoutSection{
        if (homeVM.homeData.category?.count ?? 0) > 0,homeVM.homeData.category?[selIndex].layout == "waterfall"{
...
extension SubHomeViewController: UICollectionViewDataSource,UICollectionViewDelegate{
    func numberOfSections(in collectionView: UICollectionView) -> Int {
        let textEmpty = searchTxt.text?.isEmpty ?? true
        let searchSection = searchSectionStack.isHidden
        return (homeVM.homeData.category?.isEmpty ?? true) ? 0 : (textEmpty && searchSection) ? 3 : 2
    }
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        let data = homeVM.homeData
        let textEmpty = searchTxt.text?.isEmpty ?? true
        let searchSection = searchSectionStack.isHidden
//        let prodData = (textEmpty ? data.product : homeVM.filterproducts).filter{ $0.category_id == data.category[selIndex].id }
        let prodData = (data.product!).filter{ $0.category_id == data.category?[selIndex].id }
        return section == 0 ? data.category!.count : section == 1 ? (textEmpty && searchSection) ? data.offer!.count : prodData.count : prodData.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.section == 0{
            ...
            if textEmpty && searchSection{
                ...
                if let offer = data.offer?[indexPath.item]{
...
//        let prod = (textEmpty ? data.products : homeVM.filterproducts).filter{ $0.category_id == data.category[selIndex].id }[indexPath.row]
        let prod = (data.product)?.filter{ $0.category_id == data.category?[selIndex].id }[indexPath.row]
        if data.category?[selIndex].layout == "linear"{
            ...
            if let prod{
                ...
                if let cardoffer = prod.card_offer_ids as? [String]{
                    cell.titleLbl.textColor = cardoffer.contains(sel_card_id?.id ?? "") ? .green : .BlackColor
                }
            }
            return cell
        }else{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.section == 0 && selIndex != indexPath.item{
            ...
            if textEmpty && searchSection{
                let data = homeVM.homeData
                sel_card_id = data.offer?[indexPath.item]
                collectreloadData()
            }
        }
    }
...
//        let data = homeVM.homeData
//        let filterData = data.product?.filter{ $0.category_id == data.category[selIndex].id }
